...
#include "EPotential2.h"

registerMooseObject("MooseApp", EPotential2);

template <>
InputParameters
validParams<EPotential2>()
{
  ...
  params.addParam<Real>("Dop", 0.0, "Dopant fixed concentration");
...
EPotential2::EPotential2(const InputParameters & parameters)
  : Kernel(parameters),
...
Real
EPotential2::computeQpJacobian()
{
  return -(_eps0 * _epsr) * _grad_phi[_j][_qp] * _grad_test[_i][_qp];
}

Real
EPotential2::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _e_var)
  {
